...
def item(data):
    print('一个item有多少brand,price salse collected level……NO')
    ...
    print('一个brand有多少price salse collected level……NO')
    ...
    print('一个city有多少item_price_level，item_sales_level，item_collected_level，item_pv_level...NO')
    ...
    print('一个price有多少item_sales_level，item_collected_level，item_pv_level..NO')
    ...
    print('一个item_sales_level有多少item_collected_level，item_pv_level...NO')
    ...
    print('一个item_collected_level有多少item_pv_level...YES')
...
def user(data):
    ...
    print('user_age_level对应的user_occupation_id，user_star_level...NO')
    ...
    print('user_occupation_id对应的user_star_level....NO')
...
def shop_item(data):
    print('一个shop有多少item_id,item_brand_id,item_city_id,item_price_level……NO')
    ...
    print('一个shop_review_num_level有多少item_id,item_brand_id,item_city_id,item_price_level……YES')
...
def sub(train, test, best_iter):
    ...
    test['predicted_score'] = pred
    sub1 = test[['instance_id', 'predicted_score']]
    sub=pd.read_csv("E:\\alimama\\round1_ijcai_18_test_a_20180301.txt", sep="\s+")
    sub=pd.merge(sub,sub1,on=['instance_id'],how='left')
    sub=sub.fillna(0)
    #sub[['instance_id', 'predicted_score']].to_csv('result/result0320.csv',index=None,sep=' ')
    sub[['instance_id', 'predicted_score']].to_csv('d44.txt',sep=" ",index=False)
def shijianhhh(data):
    ...
    data['day121'] = data['day12'] / data['ds']
    ...
    data['day131'] = data['day13'] / data['day12']
    del data['ds']
    return data
def sex(x):
...
def yuchuli(data):
    data['user_gender_id'] = data['user_gender_id'].apply(sex)
    data['user_age_level'] = data['user_age_level'].apply(age1)
    data['user_occupation_id'] = data['user_occupation_id'].apply(age)
    data['user_star_level'] = data['user_star_level'].apply(age)
    data['shop_review_num_level']=data['shop_review_num_level'].apply(age)
    data['shop_star_level']=data['shop_star_level'].apply(age)
    data['item_price_level']=data['item_price_level'].apply(age)
    data['item_pv_level'] = data['item_pv_level'].apply(age)
    data['item_collected_level'] = data['item_collected_level'].apply(age)
    data['item_sales_level'] = data['item_sales_level'].apply(age)
    data['context_page_id']=data['context_page_id'].apply(age)
    #data['item_brand_id'] = data['item_brand_id'].apply(age2)
    #data['shop_id'] = data['shop_id'].apply(age2)
    #data['item_city_id'] = data['item_city_id'].apply(age2)
    data['item_category_list1'] = data['item_category_list1'].apply(ah)
    return data

def zuhe1(data):

    for col in ['item_sales_level', 'item_price_level', 'item_collected_level','item_pv_level',
                'item_brand_id','shop_id','item_city_id',
                'user_gender_id','user_age_level','user_occupation_id','user_star_level',
                'shop_review_num_level', 'shop_star_level',
                'item_category_list1']:
        data[col] = data[col].astype(str)

    data['gender_age_occu']=data['user_gender_id']+data['user_age_level']+data['user_occupation_id']

    data['gender_age_occu_star'] = data['user_gender_id'] + data['user_age_level'] + data['user_occupation_id']+data['user_star_level']
    data['gender_age_occu_price'] = data['user_gender_id'] + data['user_age_level'] + data['user_occupation_id'] + data[
        'item_price_level']
    data['price_shop_review']=data['item_price_level']+data['shop_review_num_level']
    data['gender_age_occu_category'] = data['user_gender_id'] + data['user_age_level'] + data['user_occupation_id'] + data['item_category_list1']
    data['price_category']=data['user_gender_id'] + data['user_occupation_id'] +data['item_category_list1']

    #data['de_se_deliver']=data['shop_review_num_level']+data['review_map']+data['item_price_level']
    # data['de_map']+data['service_map']+data['deliver_map']
    for col in ['item_sales_level', 'item_price_level', 'item_collected_level','item_pv_level',
                'item_brand_id', 'shop_id', 'item_city_id','item_category_list1',
                'user_gender_id', 'user_age_level', 'user_occupation_id', 'user_star_level',
                'shop_review_num_level','shop_star_level',
                'gender_age_occu','gender_age_occu_price','gender_age_occu_star','price_shop_review',
                'gender_age_occu_category','price_category']:
        data[col] = data[col].astype(int)

    return data
def del_data_col(data):
    del data['de_map']
    del data['service_map']
    del data['deliver_map']
    del data['review_map']
